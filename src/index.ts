...
export async function validatePassword(password: string, options: ValidationOptions = {}): Promise<ValidationResult> {
    ...
    const charResult = allowedCharactersValidator(password, options.allowedCharacterSet);
    ...
    if (options.blacklist) {
      const blacklistResult = blacklistValidator(password, options.blacklist, options.fuzzyTolerance || 0);
